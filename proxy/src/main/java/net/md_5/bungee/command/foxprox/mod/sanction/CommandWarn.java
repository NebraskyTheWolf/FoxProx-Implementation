...
public class CommandWarn extends AbstractCommand {
    public CommandWarn() {
        super(ProxyServer.getInstance(), "warn");
    }
    ...
    protected void onCommand(CommandSender sender, String[] args) throws Exception {
        ...
        if (target.isConnected()) {
            ...
            ModerationUtils.sendMessage(ModChannel.SANCTION, ChatColor.GOLD + "" + moderator.getName() + ChatColor.YELLOW + " Warned " + ChatColor.GOLD + target.getName() + ChatColor.YELLOW + " for " + ChatColor.GOLD + reason + ChatColor.YELLOW + ".");
        } else {
